...
int check_neighbors(std::vector<std::vector<cell>>& grid, int row, int col, int cur_blob, int row_max, int col_max, int conn_type){
    ...
    if(conn_type == 4){
        count = 1;
        check_neighbors_4(grid, row, col, cur_blob+1, row_max, col_max, count);
    }
    else{
        count = 1;
        check_neighbors_8(grid, row, col, cur_blob+1, row_max, col_max, count);
    }
    return count;
}
...
int main(int argc, char** argv){
    ...
    std::cout << check_blobs(matrix, 0, row, col, conn_type) << "\n";
    return 0;
}