...
void* get_first(node_t* head_list) {
  if (!head_list) {
    return NULL;  // Add this check to prevent accessing NULL pointer
  }
...
void* get_last(node_t* head_list) {
    ...
    while (curr != NULL) {  // Changed condition to check curr itself
        if (curr->next == NULL) {
            return curr->data;
        }
        if (curr->next == curr) {
            fprintf(stderr, "Error: Circular reference detected in linked list.\n");
            return NULL;
        }
        curr = curr->next;
    }
...
void insert_last(node_t** head_list, void* to_add, size_t size) {
  ...
  void* new_data = malloc(size);  // Allocate exact size, not size - 1
  memcpy(new_data, to_add, size); // Copy the data

  // Set the new node's data
  new_element->data = new_data;
  new_element->next = NULL;       // Ensure next is initialized to NULL
  new_element->prev = NULL;       // Initialize prev (will be updated if not the first node)
  ...
  // Update pointers to insert the new element at the end
...
void* remove_last(node_t** head_list) {
  ...
  if (curr->prev) {
    ...
    *head_list = NULL;  // List becomes empty
  }
