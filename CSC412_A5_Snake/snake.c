...
int main(int argc, char** argv) {
    ...
    if (status != INIT_SUCCESS || cells == NULL || width == 0 || height == 0) {
        return EXIT_FAILURE;
    }
    ...
    // Save name length using mbstrlen (multi-byte string length)
    ...
    while (!g_game_over) {
        // Sleep for 100ms between frames
        usleep(100000);
    ...
    end_game(cells, width, height, NULL);
}